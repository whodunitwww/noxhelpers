...
local HUNGER_BAR_OFFSET = Vector2.new(5, 0);
...
function EspObject:Construct()
    ...
    self.drawings = {
        ...
        visible = {
            ...
            hungerBarOutline = self:_create("Line", { Thickness = 3, Visible = false }),
            hungerBar = self:_create("Line", { Thickness = 1, Visible = false }),
        },
        hidden = {
...
function EspObject:Update()
    ...
    self.hunger, self.maxHunger = interface.getHunger(self.player);
...
function EspObject:Render()
    ...
    visible.hungerBar.Visible = enabled and onScreen and options.hungerBar;
    visible.hungerBarOutline.Visible = visible.hungerBar.Visible and options.hungerBarOutline;

    if visible.hungerBar.Visible then
        -- We calculate positions based on the Right side (topRight and bottomRight)
        local barFrom = corners.topRight + HUNGER_BAR_OFFSET;
        local barTo = corners.bottomRight + HUNGER_BAR_OFFSET;

        local hungerBar = visible.hungerBar;
        hungerBar.To = barTo;
        -- Lerp fills from bottom (barTo) to top (barFrom) based on percentage
        hungerBar.From = lerp2(barTo, barFrom, self.hunger / self.maxHunger);
        hungerBar.Color = options.hungerColor;

        local hungerBarOutline = visible.hungerBarOutline;
        -- Reuse the HEALTH_BAR_OUTLINE_OFFSET (0,1) for vertical spacing consistency
        hungerBarOutline.To = barTo + HEALTH_BAR_OUTLINE_OFFSET;
        hungerBarOutline.From = barFrom - HEALTH_BAR_OUTLINE_OFFSET;
        hungerBarOutline.Color = parseColor(self, options.hungerBarOutlineColor[1], true);
        hungerBarOutline.Transparency = options.hungerBarOutlineColor[2];
    end
...
local EspInterface = {
    ...
    teamSettings = {
        enemy = {
            ...
            hungerBar = false,
            hungerColor = Color3.fromRGB(255, 170, 0), -- Orange/Yellowish
            hungerBarOutline = true,
            hungerBarOutlineColor = { Color3.new(), 0.5 },
        },
        friendly = {
            enabled = false,
            box = false,
...
function EspInterface.Load()
    ...
    -- FIXED LOOP: Originally assumed index 2+ were enemies.
    -- Now loops all and skips self in createObject
...
-- UPDATED: This now looks in workspace.Entities first!
function EspInterface.getCharacter(player)
    local entities = workspace:FindFirstChild("Entities")
    if entities then
        local char = entities:FindFirstChild(player.Name)
...
function EspInterface.getHealth(player)
    -- This now uses the updated getCharacter function automatically
...
function EspInterface.getHunger(player)
    local entities = workspace:FindFirstChild("Entities")
    local char = entities and entities:FindFirstChild(player.Name)
    
    if char then
        local hungerValue = char:FindFirstChild("Hunger")
        if hungerValue and hungerValue:IsA("NumberValue") then
            -- Return value clamped at 100, and the max value (100)
            return math.clamp(hungerValue.Value, 0, 100), 100
        end
    end
        return 100, 100
end
