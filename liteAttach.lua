return function(ctx)
    ...
    ------------------------------------------------------------------------
    -- Service shortcuts (real Roblox services)
    ------------------------------------------------------------------------
    -- OPT: use real services so we don't rely on ctx.Services being complete
    local Players    = b.Players   or game:GetService("Players")
    local RunService = b.RunService or game:GetService("RunService")
    local Workspace  = b.Workspace or game:GetService("Workspace")
    local HttpService = b.HttpService or game:GetService("HttpService")
    ...
    ------------------------------------------------------------------------
    -- Target discovery (with optimisations)
    ------------------------------------------------------------------------

    -- OPT: entity scanning cache to avoid constant Workspace:GetDescendants spam
    local ENTITIES_SCAN_INTERVAL = 0.5
    local lastEntitiesScanTime = 0
    local cachedEntitiesByName = {}
    ...
    local function collectEntitiesByName()
        local now = os.clock()
        if now - lastEntitiesScanTime < ENTITIES_SCAN_INTERVAL then
            return cachedEntitiesByName
        end
        ...
        cachedEntitiesByName = result
        lastEntitiesScanTime = now
        return result
    end
    ...
    local function collectStaticModels(staticConfig)
        ...
        -- OPT: simple per-config cache so we don't re-scan every call
        if staticConfig._cachedStatic and staticConfig._cachedStaticValid then
            return staticConfig._cachedStatic
        end
        ...
        staticConfig._cachedStatic = result
        staticConfig._cachedStaticValid = true
        ...
        for pattern, cfg in pairs(o) do
            if tostring(name):find(pattern, 1, true) then
                ...
                elseif type(cfg) == "table" then
                    ...
                    if cfg.horiz ~= nil then
                        a2.horizDist = tonumber(cfg.horiz) or a2.horizDist
                    elseif cfg.horizDist ~= nil then
                        a2.horizDist = tonumber(cfg.horizDist) or a2.horizDist
                    end
                end
                break
            end
    ...
    local function chooseNearestTarget()
        ...
        for _, inst in ipairs(candidates) do
            local root = t(inst)
            if root then
                -- OPT: distance^2 to avoid expensive sqrt
    ...
    local function startNoClip()
        ...
        a2._noclipHB = RunService.Stepped:Connect(applyNoClip)
    end
    ...
    local function isWithinDodgeRange(target)
        ...
        local distSq = offset.X * offset.X + offset.Y * offset.Y + offset.Z * offset.Z
        return distSq <= (a2.dodgeRange or 50) ^ 2
    end
    ...
    local function hookHumanoidForDodge(humanoid)
        ...
        local isPlayerChar = Players:GetPlayerFromCharacter(humanoid.Parent) ~= nil
        if isPlayerChar then return end

        -- OPT: if no dodge config at all, don't connect
        if not next(n) then return end
    ...
    local function rebuildDodgeMonitoring()
        ...
        -- OPT: also skip if there are no dodge-configured anims
        if not next(n) then return end
        ...
        a2._humAdded = Workspace.DescendantAdded:Connect(function(inst)
            if a2.dodgeMode and inst:IsA("Humanoid") then aw(inst) end
        end)

        a2._humRemoved = Workspace.DescendantRemoving:Connect(function(inst)
    ...
    local function goAttachLoop()
        ...
        a2._hb = RunService.RenderStepped:Connect(function()
    ...
    local function goApproachThenAttach()
        ...
        a2._hb = RunService.Heartbeat:Connect(function()
            if not a2.running then return end
            local hrp = d.humanoidRootPart
