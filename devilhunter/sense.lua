...
function EspInterface.getWeapon(player)
    -- 1. Try to get weapon from custom path: workspace.World.Entities[Name].Info.Weapon
    -- We use pcall/safe indexing to ensure it doesn't error if "World" or "Entities" is missing
    local success, weaponName = pcall(function()
        local world = workspace:FindFirstChild("World")
        local entities = world and world:FindFirstChild("Entities")
        local entity = entities and entities:FindFirstChild(player.Name)
        local info = entity and entity:FindFirstChild("Info")
        local weaponObj = info and info:FindFirstChild("Weapon")
        
        if weaponObj then
            -- If it is a Value object (StringValue, etc.), return the Value
            if weaponObj:IsA("ValueBase") then
                return tostring(weaponObj.Value)
            end
            -- If it's a model or other object, return its name (unless it's literally named "Weapon")
            if weaponObj.Name ~= "Weapon" then
                return weaponObj.Name
            end
        end
        return nil
    end)

    -- If we found a valid weapon string from the custom path, return it
    if success and weaponName and weaponName ~= "" then
        return weaponName
    end

    -- 2. Fallback: Standard Character Tool Check
